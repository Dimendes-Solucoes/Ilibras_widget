...
(function () {
  ...
  class ILibrasWidget {
    ...
    injectStyles() {
      ...
      link.rel = 'stylesheet';
      link.href = this.assetUrl('ilibras-widget.css');
      document.head.appendChild(link);
    }

    /**
     * Resolve arquivo irmão do script. Sem isto o caminho era relativo à página
     * do cliente, e logo e folha de estilo só carregavam quando o widget estava
     * na raiz do site dele.
     */
    assetUrl(arquivo) {
      const scripts = document.getElementsByTagName('script');
      for (const script of scripts) {
        if (script.src && script.src.includes('ilibras-widget.js')) {
          return script.src.replace(/ilibras-widget\.js.*$/, arquivo);
        }
      }
      return arquivo;
    }
    ...
    createWidget() {
      ...
      container.innerHTML = `
        <button type="button" id="ilibras-widget-button" class="ilibras-widget-button"
          ...
          <img src="${this.escapar(this.assetUrl('ilibras-LOGO.svg'))}" width="32" height="32" alt="" aria-hidden="true">
